...
import com.Novel.VirtualBookStore.security.CustomUserDetailsService;
...
public class PublicController {
	...
	@PostMapping("/login")
	public ResponseEntity<String> loginUser(@RequestBody loginBody login) { // we will send token here instead of
																			// credentials
		try {
			Authentication authentication = authenticationManager
					.authenticate(new UsernamePasswordAuthenticationToken(login.getEmail(), login.getPassword()));
			UserDetails userDetails = customUserDetailsService.loadUserByUsername(login.getEmail()); // i will try to
																										// achieve this
																										// using
																										// security
																										// context
			String jwt = jwtUtil.generateToken(userDetails.getUsername()); // creating token using userName as a data
	...
	public String deleteUser(@PathVariable UUID id) {
		return userCartService.deleteUserWithCart(id);
	}
